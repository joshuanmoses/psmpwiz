...
###########################################################################
#
# NAME: CyberArk PSMP Install Helper
#
# AUTHOR:  Joshua Moses <joshua.moses@cyberark.com>
#
# COMMENT: 
...
PVWAAUTH(){
read -r -p "*****(Optional) Would you like to validate the entered credentials? this will require you to input your PVWA Portal URL & Make sure FW is open on 443 [Y/N] " response
if [[ $response =~ ^([yY][eE][sS]|[yY])$ ]]
	then
		read -r -p "Please enter your PVWA Portal URL: " pvwaURL
...
creds(){
read -p "Please Enter PVWA Install Username: " adminuser
echo " "
echo "***** Please Enter PVWA Install User Password and press ENTER *****"
read -s adminpass
	if [ -z "$adminpass" ]
		then
...
clear
echo "***** To perform this task we must be able to reach your cloud portal (ie; https://user.privilegecloud.cyberark.cloud) via HTTPS/443."
echo ""
...
if [[ $response =~ ^([yY][eE][sS]|[yY])$ ]];then
		...
		echo "***** Grabbing PVWA URL from: $pvconfigurationFile"
			if [ -s $pvconfigurationFile ]; then
				...
				read -r -p "couldn't grab PVWA URL, please enter it manually (ie; https://user.privilegecloud.cyberark.cloud)." pvwaURL
				extractSubDomainFromURL=${pvwaURL%%.*}
				TrimHTTPs=${extractSubDomainFromURL#*//}
				#Check if URL belongs to UM env, otherwise use legacy.
				if [[ $pvwaURL == *"cyberark.cloud"* ]]; then
